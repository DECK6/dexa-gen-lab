#!/usr/bin/env node
/**
 * Thumbnail generator + render gate (SPEC §8-4).
 * Serves dist/ with vite preview, visits the chrome-less preview harness for every
 * sketch in thumb mode, waits for __SKETCH_READY__, and writes public/thumbs/<id>.jpg.
 * A canvas with (near) zero pixel variance counts as a render failure.
 */
...
import { listIds } from './sketch-ids.js'

const ROOT = dirname(dirname(fileURLToPath(import.meta.url)))
const THUMBS = join(ROOT, 'public/thumbs')
const PORT = 4188
const BASE = `http://localhost:${PORT}/gen/`
const SIZE = 320
const SEED = 7
const CONCURRENCY = 3
const BLANK_MAD = 0.8
...
async function shoot(page, id) {
  // Query buster forces a full document load so each sketch gets a clean GL context.
  await page.goto(`${BASE}?s=${id}#/p/${id}?seed=${SEED}&thumb=1&size=${SIZE}`, {
    waitUntil: 'load',
  })
  await page.waitForFunction(() => window.__SKETCH_READY__ === true, undefined, { timeout: 30_000 })
...
const ids = listIds()
if (!ids.length) {
  console.log('thumbs — no sketches found, nothing to render')
  process.exit(0)
}
